...
class Container : public Element {
 ...
 public:
  explicit Container(Type type, vector<Element *> elements)
      : Element(type), elements(elements) {}
};
...
class Integer : public Element {
 ...
 public:
  explicit Integer(T value, Type type = Type::Integer)
...
class String : public Element {
 ...
 public:
  explicit String(uint8_t len, const char *value, Type type = Type::String)
      : Element(type), value(string(value, len)) {}
  explicit String(string value, Type type = Type::String)
      : Element(type), value(std::move(value)) {}
  ostringstream &append(ostringstream &oss) final {
    oss << (char)this->type;
...
class SimpleAuth : public String {
 public:
  explicit SimpleAuth(uint8_t len, const char *value)
      : String(len, value, Type::SimpleAuth) {}
  explicit SimpleAuth(string value)
      : String(std::move(value), Type::SimpleAuth) {}
};
...
class Filter : public Element {
 ...
 public:
  explicit Filter(uint8_t filterTypeLen, const char *filterType,
                  uint8_t matchValueLen, const char *matchValue,
                  Type type = Type::ExtensibleMatch)
      : Element(type),
        filterType(string(filterType, filterTypeLen)),
        matchValue(string(matchValue, matchValueLen)) {}
  explicit Filter(string filterType, string matchValue,
                  Type type = Type::ExtensibleMatch)
      : Element(type),
        filterType(std::move(filterType)),
        matchValue(std::move(matchValue)) {}
  ostringstream &append(ostringstream &oss) final {
    auto _filterType = String(this->filterType,
                              static_cast<Type>(MatchingRuleAssertion::Type));
    auto _matchValue = String(
        this->matchValue, static_cast<Type>(MatchingRuleAssertion::MatchValue));
    auto extensibleMatchLength =
        _filterType.str().length() + _matchValue.str().length();
...
class Attribute : public Element {
 ...
 public:
  explicit Attribute(uint8_t attributeLen, const char *attribute)
      : Element(Type::Attribute), value(string(attribute, attributeLen)) {}
...
enum class Scope : uint8_t { BaseObject, SingleLevel, WholeSubtree };
...
class MsgBuilder {
 public:
  ...
  MsgBuilder() = default;
  static unique_ptr<Msg> build(Op *op) {
    return std::unique_ptr<Msg>(new Msg(id++, op));
  }
...
class BaseMsg {
 protected:
  Op op;
  unique_ptr<Msg> msg;
  explicit BaseMsg(Protocol::Type type)
      : op(Op(type)), msg(LDAP::MsgBuilder::build(&this->op)) {}
...
class BindRequest : public BaseMsg {
 ...
 public:
  BindRequest(string name, string password)
    ...
    this->op
        .addElement(&this->version)  // Supported LDAP version
...
class BindResponse : public BaseMsg {
 public:
  BindResponse(Protocol::ResultCode resultCode)
      : BaseMsg(Protocol::Type::BindResponse) {
    this->op.addElement(
        new BER::Integer<uint8_t>(static_cast<unsigned char>(resultCode)));
  }
  static BindResponse *parse(string msg) {
    std::vector<uint8_t> vec(msg.begin(), msg.end());

    for (uint8_t x : vec) std::cout << x << std::endl;
...
class SearchRequest : public BaseMsg {
 ...
 public:
  SearchRequest(string baseObject, string filterType, string filterValue,
                string attribute,
                Protocol::SearchRequest::Scope scope =
                    Protocol::SearchRequest::Scope::SingleLevel,
                Protocol::SearchRequest::DerefAliases derefAliases =
                    Protocol::SearchRequest::DerefAliases::NeverDerefAliases,
