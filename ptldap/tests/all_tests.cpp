...
TEST_CASE( "Parse BER::Bool", "[BER::Bool]" ) {
    ...
    auto ber_bool_false_parsed = BER::Bool::parse("\x01\x01\x00"s).value;
    auto ber_bool_true_parsed = BER::Bool::parse("\x01\x01\x01"s).value;
...
TEST_CASE( "Parse BER::Integer", "[BER::Integer]" ) {
    ...
    auto ber_integer_u8_parsed = BER::Integer::parse("\x02\x01\x42"s).value;
...
TEST_CASE( "Parse BER::Enum", "[BER::Enum]" ) {
    ...
    auto ber_enum_type_parsed = BER::Enum<BER::Type>::parse("\x0a\x01\x0a"s).value;
...
TEST_CASE( "Generate a BindRequest", "[bindRequest]" ) {
    ...
    for (size_t i = 0 ; i < expected_str.size(); i++) {
        int expected_chr = (uint8_t)expected_str[i];
        int msg_chr = (uint8_t)msg_str[i];
...
TEST_CASE( "Parse a BindRequest", "[bindRequest]" ) {
    ...
    INFO("Parsed size: " << bind_request.length << " vs Expected size: " << msg_str.size());
    REQUIRE( bind_request.value != nullptr );
    REQUIRE( bind_request.length == msg_str.size() );

    INFO("Username: " << bind_request.value->name.value);
    INFO("Password: " << bind_request.value->password.value);
    REQUIRE( bind_request.value->name.value == expected_login );
    REQUIRE( bind_request.value->password.value == expected_password );
}
