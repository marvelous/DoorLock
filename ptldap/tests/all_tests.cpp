...
TEST_CASE("Generate a BindRequest", "[bindRequest]") {
  ...
  auto expected_str =
      "\x30\x21\x02\x01\x01\x60\x1c\x02\x01\x03\x04\x0a\x74\x65\x73\x74\x5f\x6c\x6f\x67\x69\x6e\x80\x0b\x74\x65\x73\x74\x5f\x70\x61\x73\x73\x77\x64"s;
  ...
  REQUIRE(memcmp(expected_str.c_str(), msg_str.c_str(), expected_str.size()) ==
          0);
}
...
TEST_CASE("Parse a BindResponse", "[bindResponse]") {
  LDAP::MsgBuilder::reset_id();

  auto response_str =
      "\x30\x0c\x02\x01\x01\x61\x07\x0a\x01\x00\x04\x00\x04\x00"s;

  auto msg_str = LDAP::BindResponse::parse(response_str);
